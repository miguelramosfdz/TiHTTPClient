...
+(UIColor*)webColorNamed:(NSString*)colorName
{
	...
	if (colorLookup == nil)
	{
		...
		colorLookup = [[NSMutableDictionary alloc] initWithObjectsAndKeys:
					   ...
					   [TiWebColor colorForHex:@"0ff"],@"aqua",
					   [TiWebColor colorForHex:@"f0f"],@"fuchsia",
					   [TiWebColor colorForHex:@"0f0"],@"lime",
					   [TiWebColor colorForHex:@"800"],@"maroon",
					   [TiWebColor colorForHex:@"FFC0CB"],@"pink",
					   [TiWebColor colorForHex:@"000080"],@"navy",
					   [TiWebColor colorForHex:@"c0c0c0"],@"silver",
					   [TiWebColor colorForHex:@"808000"],@"olive",
					   [TiWebColor colorForHex:@"008080"],@"teal",
	...
	result = [TiWebColor colorForHex:colorName];
	
	if (result == nil)
	{
		result = [TiWebColor colorForRGBFunction:colorName];
	}
...
+(UIColor*)colorForHex:(NSString*)hexCode
{
	...
	{
		PELog(@"[WARN] Hex color passed looks invalid: %@",hexCode);
        return nil;
	}
    unsigned value = 0;
