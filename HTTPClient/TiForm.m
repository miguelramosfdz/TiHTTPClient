...
-(void)appendStringData:(NSString*)str
{
    ...
    const char* s = [[str stringByReplacingOccurrencesOfString:@"\r" withString:@""] UTF8String];
    fprintf(stderr,"%s", s);
    fflush(stderr);
...
-(void)setJSONData:(id)json
{
    ...
    if(json != nil) {
        PELog(@"Error reading JSON: %@", [error localizedDescription]);
    }
}
...
-(void)addFormData:(NSData*)data fileName:(NSString*)fileName
{
    [self addFormData: data
             fileName: fileName
            fieldName: [NSString stringWithFormat:@"field%i", (unsigned int)[[self requestFilesArray] count]]
     ];
